...
* `8.0.0` &mdash; **2018-08-03** **breaking**
    ...
    * Remove all functional-style utils. I recommend using Ramda or Lodash for
      FP, since those libraries are better tested and maintained. Removed
      functions:
        * all.js
        * and.js
        * any.js
...
* `7.0.3` &mdash; **2018-05-30**
    * Fix `words` to return empty array from empty string
      ([@TomPridham](https://github.com/TomPridham))
...
* `6.0.0` &mdash; **2018-02-10** **breaking**
    * Remove `getLengthOfItemsInArray` and `getLengthOfCharactersInString` (use
      `countIn`)
...
* `2.0.0` &mdash; **2017-07-29** &mdash; *breaking*
    ...
    * Refactored `safeGet`, which now takes an access string first and an
      optional fallback, then the object.
...
* `0.9.0` &mdash; **2017-05-27** &mdash; *breaking*
    * All filenames are now `lisp-cased`, so if you import them directly you may
      need to change your imports.
    * `objectToString` now `call`s, instead of needing to do that everywhere
      it's used
...
* `0.8.0` &mdash; **2017-05-24** &mdash; *breaking*
    ...
    * `getNodeModules` now returns an array of locally installed modules,
...
* `0.7.0` &mdash; **2017-04-05** &mdash; *important*
    * If you've installed in the past eight days (since `0.6.17`
      **2017-03-28**), you're probably seeing a lot of annoying logs! I did
      something dumb. Please update!
...
* `0.2.0` &mdash; **2017-01-30** &mdash; *breaking*
    * All functions now in top level (no more `import { fp } from 'zeelib'` to
      use `zipWith`, just `import { zipWith } from 'zeelib'`).
